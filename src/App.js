...
this.setState({
  ...
  render() {
    var movieList =this.state.movies.map((movie,i)=>{
      ...
      return(
        <Movie likeClickParent={this.likeClick}movieName={movie.title} movieDesc={movie.overview} idMovieDB={movie.id} movieImg={'https://image.tmdb.org/t/p/w500'+movie.poster_path} isliked={isliked}viewOnlyLike={this.state.viewOnlyLike} key={i}/>
      );
    })
          ...
          <Navbar color="dark" dark expand="md" fixed="top">
            ...
            <Collapse isOpen={this.state.isOpenNavBar} navbar>
              <Nav className="" navbar>
                ...
                <NavItem>
                  <NavLink href='#' onClick={this.handleClickLikeOn}  style={{color: "#FFFFFF", marginRight: 10}}>My Movies</NavLink>
                </NavItem>
                <Button id="Popover1" onClick={this.togglePopOver} color="secondary">{moviesCount}{moviesCount > 1 ? ' films' : ' film'}</Button>
